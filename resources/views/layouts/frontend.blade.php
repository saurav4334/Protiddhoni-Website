...
<head>
  ...
  <style>
    ...
    .hero-gradient {
      background: radial-gradient(1200px 600px at 10% -10%, rgba(0, 156, 222, .25), transparent 60%), radial-gradient(900px 500px at 90% 10%, rgba(255, 196, 57, .25), transparent 60%), linear-gradient(180deg, #ffffff 0%, #f7f9fc 100%);
    }
    ...
    .btn-primary {
      ...
      gap: .5rem;
      transition: .2s;
      box-shadow: 0 10px 24px -10px rgba(0, 112, 186, .6);
    }
    ...
    .btn-gold {
      ...
      gap: .5rem;
      transition: .2s;
      box-shadow: 0 10px 24px -10px rgba(255, 196, 57, .6);
    }
    ...
    .card:hover {
      border-color: #009CDE;
      box-shadow: 0 20px 50px -20px rgba(0, 112, 186, .2);
    ...
    .glow-ring {
      box-shadow: 0 0 0 0 rgba(0, 156, 222, .6);
      animation: glow 2.5s infinite;
    }

    @keyframes glow {
      0% {
        box-shadow: 0 0 0 0 rgba(0, 156, 222, .5)
      }

      70% {
        box-shadow: 0 0 0 18px rgba(0, 156, 222, 0)
      }
    ...
    .feature-icon {
      ...
      background: linear-gradient(135deg, #eef3f9, #E0F4FB);
      color: #0070BA;
    }

    .dot-grid {
      background-image: radial-gradient(#c3cad9 1px, transparent 1px);
    ...
    .ring-gold {
      box-shadow: 0 0 0 4px #FFF9E6, 0 0 0 5px #FFC439;
    }
  </style>
  @stack('styles')
</head>
  ...
  <header class="fixed top-0 inset-x-0 z-50 bg-white/80 backdrop-blur-xl border-b border-surface-200">
    <nav class="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
      <a href="{{ url('/') }}" class="flex items-center gap-2">
        <div class="w-9 h-9 rounded-xl mesh-blue flex items-center justify-center ring-gold">
          <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
            <path
              d="M12 2a3 3 0 00-3 3v7a3 3 0 006 0V5a3 3 0 00-3-3zM5 10a1 1 0 112 0 5 5 0 0010 0 1 1 0 112 0 7 7 0 01-6 6.92V20h3a1 1 0 110 2H8a1 1 0 110-2h3v-3.08A7 7 0 015 10z" />
          </svg>
        </div>
        <div>
      ...
      <div class="hidden lg:flex items-center gap-1">
        <a href="{{ url('/voice-broadcast') }}"
          class="nav-link {{ request()->is('voice-broadcast') ? 'active' : '' }}">Broadcast</a>
        <a href="{{ url('/voice-otp') }}" class="nav-link {{ request()->is('voice-otp') ? 'active' : '' }}">Voice
          OTP</a>
        <a href="{{ url('/voice-survey') }}" class="nav-link {{ request()->is('voice-survey') ? 'active' : '' }}">Voice
          Survey</a>
      ...
      <div class="flex items-center gap-3">
        ...
        <a href="{{ url('https://dashboard.protiddhoni-bd.com/signup') }}" class="btn-primary !py-2 !px-4 !text-sm">Get
          Started →</a>
      </div>
    </nav>
  </header>
  ...
  <footer class="bg-ink-900 text-white pt-16 pb-10">
    <div class="max-w-7xl mx-auto px-6 grid md:grid-cols-2 lg:grid-cols-5 gap-10">
      <div class="lg:col-span-2">
        ...
        <p class="mt-4 text-ink-300 text-sm max-w-xs">Voice broadcasting made effortless for modern Bangladeshi
          businesses. Reach thousands in seconds.</p>
        <div class="mt-6 flex gap-3">
          <a href="#"
            class="w-9 h-9 rounded-lg bg-white/10 hover:bg-paypal-gold hover:text-paypal-navy flex items-center justify-center">f</a>
          <a href="#"
            class="w-9 h-9 rounded-lg bg-white/10 hover:bg-paypal-gold hover:text-paypal-navy flex items-center justify-center">𝕏</a>
          <a href="#"
            class="w-9 h-9 rounded-lg bg-white/10 hover:bg-paypal-gold hover:text-paypal-navy flex items-center justify-center">in</a>
        </div>
      </div>
      <div>
    ...
    <div
      class="mt-12 pt-6 border-t border-white/10 max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-3 text-xs text-ink-300">
