...
<style>
  ...
  .hero{padding:60px 0 32px;background:linear-gradient(180deg,#fff 0%,var(--bg) 100%)}
  .toc-link{display:flex;gap:.6rem;padding:.5rem .8rem;border-radius:10px;font-size:.85rem;font-weight:600;color:var(--mute)}
  .toc-link:hover{background:#eef3f9;color:var(--navy)}
  .toc-link.active{background:var(--cream);color:#8b6b00}
  .step-num{width:48px;height:48px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-weight:800;color:#fff;font-size:1.2rem;flex-shrink:0;background:linear-gradient(135deg,var(--navy),var(--blue))}
  .mock{background:#fff;border:1px solid var(--line);border-radius:14px;box-shadow:0 30px 60px -30px rgba(0,48,135,.25);overflow:hidden}
  .mock-bar{background:var(--ink);color:#fff;padding:.55rem .9rem;font-size:.7rem;font-weight:600;display:flex;gap:.5rem;align-items:center}
</style>
@endpush
  ...
  <div class="max-w-7xl mx-auto px-6">
    ...
    <div class="grid lg:grid-cols-2 gap-12 items-center">
      <div class="space-y-4 reveal">
        <p class="text-ink-500 leading-relaxed">Login using your admin URL (e.g., <code>/admin</code>). You will land on the Dashboard where you can see your site\'s health at a glance.</p>
      ...
      <div class="mock reveal">
        <div class="mock-bar">
          <div class="flex gap-1.5"><i class="w-2 h-2 rounded-full bg-red-400"></i><i class="w-2 h-2 rounded-full bg-yellow-400"></i><i class="w-2 h-2 rounded-full bg-green-400"></i></div>
          <span class="opacity-70 ml-2">voicereach.com/admin</span>
        </div>
...
<section id="post" class="py-24 bg-white border-y border-surface-100">
  <div class="max-w-7xl mx-auto px-6">
    ...
    <div class="grid lg:grid-cols-2 gap-12 items-center">
      ...
      <div class="space-y-4 reveal">
        <p class="text-ink-500 leading-relaxed">The editor is fully WYSIWYG. You can format text, add images, and set SEO parameters in the sidebar.</p>
