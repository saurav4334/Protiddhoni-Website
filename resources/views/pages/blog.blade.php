...
<title>Blog & Insights — Protiddhoni</title>
<meta name="description" content="Voice marketing, OTP, and survey insights for Bangladesh. Carrier playbooks, conversion data, regulation updates, and Banglish UX learnings — all in one place." />
...
<section class="hero">
  <div class="container">
    ...
    <p class="lede">Carrier deliverability data, OTP conversion benchmarks, regulatory updates from BTRC, Banglish UX patterns, and the messy on-the-ground learnings from 280+ teams using Protiddhoni. Not theory — practice.</p>
    <div class="hero-meta">
            ...
            <div class="post-thumb" style="background:linear-gradient(135deg,#003087,#1a3a8c)">
              ...
              <svg viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg">
                ...
                <g font-family="JetBrains Mono" font-size="11" fill="#5fd4ff">
                  ...
                  <text x="20" y="60">  https://api.protiddhoni.bd/v1/calls \</text>
          ...
          <article class="post-card reveal">
            <div class="post-thumb" style="background:linear-gradient(135deg,#06112d,#003087)">
              ...
              <svg viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg">
                ...
                <text x="80" y="100" fill="#fff" font-family="Plus Jakarta Sans" font-size="14" font-weight="600">Protiddhoni replaced</text>
            ...
            <div class="post-body">
              ...
              <h3>How Pathao moved 14M monthly OTPs to Protiddhoni in three weeks — without one missed login</h3>
  ...
  <div class="container">
    <div class="cta-inner">
      <div class="reveal">
        ...
        <p>Spin up a free Protiddhoni account and run your first 100 calls today — no credit card, full Bangla support, and a real engineer on Slack if you get stuck.</p>
...
(function () {
  ...
  CMS.init({ base: CMS_BASE, page: 'blog' });

  // Render posts from /api/posts into .posts-grid (replaces hardcoded fallback).
  // If API fails, the static cards stay (graceful fallback).
  const grid = document.querySelector('.posts-grid');
  if (!grid) return;

  // Map category label → slug (kebab-case)
  function slugify(s) {
    return s.toLowerCase().replace(/[^\w]+/g, '-').replace(/^-|-$/g, '');
  }

  function postCard(post) {
    const cat = post.category ? `<span class="cat">${post.category.name}</span>` : '';
    const gradient = post.hero_gradient === 'gold-orange'
      ? 'linear-gradient(135deg,#FFC439,#FF8E2C)'
      : 'linear-gradient(135deg,#003087,#0070BA)';
    const bg = post.featured_image_url
      ? `background-image:url('${post.featured_image_url}');background-size:cover;background-position:center`
      : `background:${gradient}`;
    const author = post.author || { name: '', initials: '' };
    const initials = (author.name || '').split(' ').slice(0,2).map(s => s[0] || '').join('').toUpperCase();
    ...
    return `
      <article class="post-card reveal in">
        <a href="/blog-post?slug=${encodeURIComponent(post.slug)}" class="post-thumb" style="${bg};display:block">
          ${cat}
        </a>
        <div class="post-body">
          <div class="meta"><span>${date}</span><span>· ${minutes} min read</span></div>
          <h3>${post.title}</h3>
          <p>${post.excerpt || ''}</p>
          <div class="author-row">
            ...
            <a class="read-arrow" href="/blog-post?slug=${encodeURIComponent(post.slug)}">→</a>
          </div>
        </div>
      </article>
    `;
  ...
  window.refreshPosts = async function (categoryLabel) {
    const params = { per_page: 12 };
    if (categoryLabel && categoryLabel !== 'All') {
      params.category = slugify(categoryLabel);
    }
    try {
      const { data } = await CMS.fetchPosts(params);
      if (Array.isArray(data) && data.length > 0) {
        grid.innerHTML = data.map(postCard).join('');
      }
    } catch (err) {
      console.warn('[blog] post fetch failed — keeping static fallback.', err);
    }
  };

  // Fire on load
  window.refreshPosts('All');
})();
</script>
</body>
</html>
